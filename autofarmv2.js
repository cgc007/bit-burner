...
export async function main(ns) {
	...
	*/
	...
	await ns.write(
		files[0],
		`/** @param {import(".").NS} ns */export async function main(ns) { if (ns.args.length > 1 ) { await ns.sleep(ns.args[1]); } await ns.weaken(ns.args[0]); }`,
		"w"
	);
	await ns.write(
		files[1],
		`/** @param {import(".").NS} ns */export async function main(ns) { if (ns.args.length > 1 ) { await ns.sleep(ns.args[1]); } await ns.grow(ns.args[0]); }`,
		"w"
	);
	await ns.write(
		files[2],
		`/** @param {import(".").NS} ns */export async function main(ns) { if (ns.args.length > 1 ) { await ns.sleep(ns.args[1]); } await ns.hack(ns.args[0]); }`,
		"w"
	);
	var serverList;
	var targetList;
	...
	if (false) {
		...
		httpworm();
		sqlinject();
	} //So that the game can calculate the cost before I call them using text, else RAM cost bypass error
	if (ns.args.length == 0) {
	...
	async function scanExes() {
		...
		if (!ns.getPlayer().tor && playerMoney() > 2e5) {
			ns.exec("buy-tor.js", "home", 1);
		}
		//Buy Programs if not owned
		if (playerMoney() > 5e5 && !ns.fileExists("BruteSSH.exe")) {
			ns.exec("buy-program.js", "home", 1, "BruteSSH.exe");
		}
		if (playerMoney() > 1.5e6 && !ns.fileExists("FTPCrack.exe")) {
			ns.exec("buy-program.js", "home", 1, "FTPCrack.exe");
		}
		if (playerMoney() > 5e6 && !ns.fileExists("relaySMTP.exe")) {
			ns.exec("buy-program.js", "home", 1, "relaySMTP.exe");
		}
		if (playerMoney() > 30e6 && !ns.fileExists("HTTPWorm.exe")) {
			ns.exec("buy-program.js", "home", 1, "HTTPWorm.exe");
		}
		if (playerMoney() > 250e6 && !ns.fileExists("SQLInject.exe")) {
			ns.exec("buy-program.js", "home", 1, "SQLInject.exe");
		}
		if (playerMoney() > 5e5 && !ns.fileExists("DeepscanV1.exe")) {
			ns.exec("buy-program.js", "home", 1, "DeepscanV1.exe");
		}
		if (playerMoney() > 25e6 && !ns.fileExists("DeepscanV2.exe")) {
			ns.exec("buy-program.js", "home", 1, "DeepscanV2.exe");
		}
		if (playerMoney() > 1e6 && !ns.fileExists("AutoLink.exe")) {
			ns.exec("buy-program.js", "home", 1, "AutoLink.exe");
		}
	...
	function logBalance(server) {
		//For the balance section in display
		return (
			[ns.nFormat(ns.getServerMoneyAvailable(server), "0a")] +
			" / " +
			[ns.nFormat(ns.getServerMaxMoney(server), "0a")] +
			" : " +
	...
	async function log() {
		...
		ns.print(
			"║ EXE " +
				exes.length +
				"/5 ║ HOSTS " +
				ns.nFormat(hostList.length, "0").padEnd(2) +
				" ║ TARGETS " +
				ns.nFormat(targetList.length, "0").padEnd(2) +
				"        ║"
		);
		if (contractLocation.length > 0) {
			ns.print("╠══════╣ " + contractLocation + " ╠".padEnd(33 - contractLocation.length - 1, "═") + "╣");
		}
		if (manager) {
	...
	async function scanServers() {
		//Puts all unique servers into serverList
		...
		while (scanIndex <= serverCount[depth] - 1) {
			...
			for (let i = 0; i <= results.length - 1; i++) {
				if (results[i] != "home" && !serverList.includes(results[i])) {
					...
					if ((temp && pServers) || !temp) {
						//If the server is player owned and they allowed, or if it's not player owned
						serverList.push(results[i]);
						serverCount[depth + 1]++;
					}
				}
			}
	...
	async function checkServers() {
		//Sorts servers into lists based on RAM and money/hack time ratio: hostList and targetList
		let backdoorThisLoop = false;
		targetList = [];
		hostList = [];
		totalRam = ns.getServerMaxRam("home");
		hostList.push([ns.getServerMaxRam("home"), "home"]);
		...
		for (let i = 0; i <= serverList.length - 1; i++) {
			...
			if (ns.getServerMoneyAvailable(cTarget) > 0 || (ns.getServerMaxRam(cTarget) > 2 && ns.hasRootAccess(cTarget))) {
				...
				if (ns.getServerMoneyAvailable(cTarget) != 0 && !targetList.includes(cTarget) && ns.getServerRequiredHackingLevel(cTarget) <= ns.getHackingLevel()) {
					//And if server has money -> targetList, we only want servers with money that we can hack
					targetList.push(temp);
					targetList = arraySort(targetList); //Pushes to list and sorts list by profit
				}
				temp = ns.getPurchasedServers().includes(cTarget);

				if (ns.getServerMaxRam(cTarget) > 2 && !hostList.includes(cTarget) && ((temp && pServers) || !temp) && !cTarget.includes("hacknet-node")) {
					//If server has RAM and is allowed -> hostList]
					hostList.push([ns.getServerMaxRam(cTarget), cTarget]);
					hostList = arraySort(hostList); //Pushes to hostList and sorts by amount of RAM
	...
	async function hackAll() {
		//Dedicates high RAM servers to attack high profit per second servers
		let tarIndex = 0;
		for (let i = 0; i <= hostList.length - 1; i++) {
			if (tarIndex > Math.min(targetList.length - 1, 10)) {
				tarIndex = 0;
			}
			let hHost = hostList[i][1];
			let hTarget = targetList[tarIndex][1];
			let freeRam = ns.getServerMaxRam(hHost) - ns.getServerUsedRam(hHost);
			if (hHost == "home") {
				// special processing for home server
				if (ns.isRunning("gang-manager.js", hHost)) {
					freeRam -= ns.getScriptRam("gang-manager.js", hHost);
				}
			...
			if (freeRam >= 2) {
				...
				let bThreads = 0;
				if (ns.getServerSecurityLevel(hTarget) > ns.getServerMinSecurityLevel(hTarget) + 5) {
					//Security target here
					latest[1][0] = hHost;
					latest[1][1] = hTarget;
					let weakThreads = (ns.getServerSecurityLevel(hTarget) - ns.getServerMinSecurityLevel(hTarget)) / 0.05;
					if (weakThreads < threads) {
						//We have more threads available than needed to drop to minimum sec
						...
						if (nThreads > 1) {
							...
							gThreads = Math.ceil(
								Math.min(ns.growthAnalyze(hTarget, Math.max(ns.getServerMaxMoney(hTarget) / (ns.getServerMoneyAvailable(hTarget) + 1), 1)), gThreads)
							);
							let wThreads = Math.max(gThreads * 0.004, 1);
							ns.exec("weak.js", hHost, wThreads, hTarget);
							if (gThreads > 0) {
								ns.exec("grow.js", hHost, gThreads, hTarget);
							}
					...
					} //if we need to weaken this target more, keep the target the same for the next one
				} else if (ns.getServerMoneyAvailable(hTarget) < ns.getServerMaxMoney(hTarget) * 0.75) {
					//Server money target here
					latest[0][0] = hHost;
					latest[0][1] = hTarget;
					...
					if (freeRam >= 1.75 * 3) {
						...
						latest[2][0] = hHost;
						latest[2][1] = hTarget;
						let tTake = 0.5; // percentage to take per round of the hack
						...
						let gThreads = Math.ceil(ns.growthAnalyze(hTarget, 1 / tTake + 0.15)); //Grow money lost back + 15%
						...
						while (parseFloat(ns.hackAnalyze(hTarget)) * threads > 0.1) {
							threads--;
							bThreads++;
						} //Hack limit here
						latest[2][0] = hHost;
						...
						if (bThreads > 0) {
							ns.exec("weak.js", hHost, bThreads, hTarget);
						}
						tarIndex++;
					}
				}
	...
	async function hnManagerold() {
		...
		function check(q) {
			return eval(q < playerMoney() / 1.1);
		} //Bigger number = tighter money restriction :)
		...
		for (let i = 0; i < ns.hacknet.numNodes(); i++) {
			for (let n = 0; n < mode.length; n++) {
				if (check(ns.hacknet["get" + mode[n] + "UpgradeCost"](i))) {
					if ((mode[n] == "Cache" && ns.hacknet.numHashes() / ns.hacknet.hashCapacity() > 0.8) || mode[n] != "Cache") {
						//only do cache upgrades if we're near the ceiling of hashes
	...
	async function joinFactions() {
		...
		for (let i = factionJoinable.length - 1; i >= 0; i--) {
			if (!nonautofaction.includes(factionJoinable[i])) {
				//If the joinable faction isn't in the unjoinable list
				if (ns.joinFaction(factionJoinable[i])) {
					//join the faction
	...
	while (true) {
		...
		if (loopNum % 120 == 0) {
			...
			} else if ((ns.isRunning("better-crime.js", "home") || ns.isRunning("better-crime.js", "home", "homicide")) && ns.heart.break() <= -54000) {
